const getList = async () => {
    ...
    fetch(url, {
      ...
      .then((data) => {
        data.pacientes.forEach(item => insertList(item.nome, item.valor, item.diadasemana, item.endereco, item.comentario))
      })
      .catch((error) => {
...
const postItem = async (inputPaciente, inputValor, inputDia, inputEndereco, inputComent) => {
  ...
  const newItem = () => {
    ...
    if (inputPaciente === '') {
      ...
      insertItem(inputPaciente, inputValor, inputDia, inputEndereco, inputComent)
      postItem(inputPaciente, inputValor, inputDia, inputEndereco, inputComent)
      alert("Paciente adicionado!")
    }
  }
  
  const insertItem = (nome, valor, diadasemana, endereco, comentario) => {
    var item = [nome, valor, diadasemana, endereco, comentario];
    var table = document.getElementById('myTable');
...
fetch ("http://127.0.0.1:5000/paciente?nome=", {
  method: 'PUT',
  body: JSON.stringify({
