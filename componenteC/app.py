...
# definindo tags
home_tag = Tag(name="Documentação", description="Documentação em SwaggerSS")
...
@app.put('/paciente', tags=[paciente_tag],
         responses={"200": PacienteViewSchema, "404": ErrorSchema})
def atualiza_paciente(query: PacienteAtualizaSchema):
    """Atualiza o banco de dados dos pacientes pelo nome

    Retorna uma representação dos pacientes e informações atualizados
    """
    paciente_nome = unquote(unquote(query.nome))
    print(paciente_nome)
    logger.debug(f"Coletando paciente pelo nome")
    # criando conexão com a base
    session = Session()
    # fazendo a busca
    paciente = session.query(Paciente).filter(Paciente.nome == paciente_nome).first()
    ...
    if not paciente:
        # se o paciente não foi encontrado
        error_msg = "Paciente não encontrado na base :/"
        logger.warning(f"Erro ao buscar paciente, {error_msg}")
        return {"mesage": error_msg}, 404
    else:
        logger.debug(f"Paciente atualizado: '{paciente.nome}'")
        # retorna a representação de paciente
        return apresenta_paciente(paciente), 200
